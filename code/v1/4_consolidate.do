...
mer m:1 country year yearb using "temp/2democracy/tot/dem",keepus(dem*) keep(1 3) nogen
...
keep svyid ${placebo} 
compress
sa "temp/1survey/placebo",replace		
...
*B1. ASIANBAROMETER
*=============================================================================*/
u "raw/1survey/2asianBarometer/data",replace
...
mer m:1 country year yearb using "temp/2democracy/tot/dem",nogen keep(1 3) keepus(dem*)
mer m:1 country using "raw/CountryCod",keep(1 3) nogen keepus(continent region)
...
drop t0
compress
sa "temp/1survey/asianbarometer",replace

/*==============================================================================
*B1.2. COUNTRY-YEARB-YEAR ASIANBAROMETER
*=============================================================================*/
*Country-Year born-Year
u "temp/1survey/asianbarometer",clear
...
compress
sa "temp/1survey/censusCountryYearbYearasia",replace


/*==============================================================================
*B2. LAPOP
*=============================================================================*/
u "raw/1survey/3lapop/data",replace
decode wave,g(svy)
...
compress
sa "temp/1survey/lapop",replace

/*==============================================================================
*B2.2. COUNTRY-YEARB-YEAR LAPOP
*=============================================================================*/
*Country-Year born-Year
u "temp/1survey/lapop",clear
keep country yearb year
keep if !missing(country,yearb,year)
...
la da "Individuals census by country, year of birth and year of survey"
compress
sa "temp/1survey/censusCountryYearbYearlapop",replace


/*==============================================================================
*B3. LATINOBAROMETER
*=============================================================================*/
u "raw/1survey/4latinobarometer/data",replace
ren dembetter t0
...
compress
sa "temp/1survey/latinobarometer",replace

/*==============================================================================
*B3.2. COUNTRY-YEARB-YEAR LATINOBAROMETER
*=============================================================================*/
*Country-Year born-Year
u "temp/1survey/latinobarometer",clear
keep country yearb year
keep if !missing(country,yearb,year)
...
la da "Individuals census by country, year of birth and year of survey"
compress
sa "temp/1survey/censusCountryYearbYearlat",replace
*/
...
la data "ESS"
compress
sa "temp/1survey/ess",replace

/*==============================================================================
*B4.2. COUNTRY-YEARB-YEAR ESS
*=============================================================================*/
*Country-Year born-Year
u "temp/1survey/ess",clear
keep country yearb year
keep if !missing(country,yearb,year)
...
replace countryofbirth = "Turkey" if birth_country == "TR"
...
merge m:1 country year yearb using "temp/2democracy/tot/dem", nogen keep(1 3) keepus(dem*)
...
*C1. COHORT EVENT
*=============================================================================*/
u if dem!=. using "temp/2democracy/panelDem1",clear
...
compress
sa "temp/2democracy/event/eventsDem1",replace
...
joinby country using "temp/2democracy/event/eventsDem1"
...
compress
sa "temp/2democracy/event/cohortsEvent",replace


*EXPOSURE TO EVENT BY COHORT AND NUMBER OF YEARS
u "temp/2democracy/event/cohortsEvent",clear
qui foreach v of varlist demevent*{
	noi di "`v'"
	levelsof `v'
...
compress
savezipped using "temp/2democracy/event/cohortsEventValue",replace
...
foreach n of numlist 5{
	...
	compress
	save "temp/2democracy/event/cohortsEventValue`n'",replace
}


*EXPOSURE TO EVENT BY COHORT SUCCESS
u "temp/2democracy/event/eventsDem1",replace
...
qui forv s=1/4{
	...
	compress
	sa "temp/2democracy/event/cohortsEventSuc`s'",replace
}
