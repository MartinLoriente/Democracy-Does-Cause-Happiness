...
u "${temp}/ivs",replace
mer m:1 country yearb year using "${temp}/2_democracy/tot/bef",nogen keep(1 3)
forv c=1/1{
forv s=1/2{
	forv x=1/2{
		forv y=1/5{
...
Figure A1: Democracy and Exposure to Democracy by Year/Cohort
*=============================================================================*/
use "${temp}/2_democracy/panelDem2", clear
keep if year>=1900
...
sa "${temp}/2_democracy/tot/dem_full",replace

* Plot
use "${temp}/2_democracy/tot/dem_full", clear
...
twoway line dem yearb, c(L) lcolor(gs12) lwidth(vthin) ///
|| line dem yearb if country=="ARGENTINA", c(L) lcolor(eltblue) lwidth(thick) ///
|| line dem yearb if country=="SOUTH KOREA", c(L) lcolor(dkgreen) lwidth(thick) lpattern(dash_dot) ///
|| line dem yearb if country=="TURKIYE", c(L) lcolor(maroon) lwidth(thick) lpattern(dash) ///
|| line dem yearb if country=="UNITED STATES", c(L) lcolor(navy) lwidth(thick) lpattern(shortdash) ///
...
forv suc=1/4{	
	u "${temp}/2_democracy/sucM/bef/suc`suc'",clear
	ren (aut* dem* suc* uns*) b=
	mer 1:m country yearb using "${temp}/2_democracy/sucM/dem/suc`suc'iy",nogen
...
forv suc=1/4{	
	u "${temp}/2_democracy/sucM/bef/suc`suc'",clear
	ren (aut* dem* suc* uns*) b=
	mer 1:m country yearb using "${temp}/2_democracy/sucM/dem/suc`suc'lt",nogen
	mer 1:m country yearb year using "${temp}/ivs",keep(2 3) nogen
...
u "${temp}/3_addVars/heM/he",replace
...
use "${temp}/ivs", clear
keep if age>24
merge m:1 country yearb year using "${temp}/2_democracy/tot/instiy.dta", keep(1 3) nogen keepus(z1dem1 z2dem2)
rename (z1dem1 z2dem2) (zdem1 zdem2)
merge m:1 country yearb using "${temp}/2_democracy/tot/instBef", keep(1 3) nogen
forvalues s = 1/2 {
	forvalues x = 1/2 {
		forvalues y = 1/5 {
			local dep = word("${a}", `y')
...
use "${temp}/ivs", clear
merge m:1 country yearb year using "${temp}/2_democracy/tot/instlt.dta", keep(1 3) nogen keepus(z1dem1 z2dem2)
rename (z1dem1 z2dem2) (zdem1 zdem2)
merge m:1 country yearb using "${temp}/2_democracy/tot/instBef", keep(1 3) nogen
forvalues s = 1/2 {
	forvalues x = 1/2 {
...
forv suc=1/4{
	u "${temp}/2_democracy/sucM/instBef/suc`suc'",clear
	ren (aut* dem* suc* uns*) zb=
	mer 1:m country yearb using "${temp}/2_democracy/sucM/inst/suc`suc'iy",nogen
...
forv suc=1/4{
	u "${temp}/2_democracy/sucM/instBef/suc`suc'",clear
	ren (aut* dem* suc* uns*) zb=
	mer 1:m country yearb using "${temp}/2_democracy/sucM/inst/suc`suc'",nogen
	ren (aut* dem* suc* uns*) z=
	mer 1:m country yearb year using "${temp}/ivs",keep(2 3) nogen
	forv s=1/2{
		forv x=1/1{
...
use "${temp}/ivs", clear
keep if age>24
merge 1:1 svyid using "${temp}/1_survey/placebo", nogen keep(1 3)
merge m:1 country yearb year using "${temp}/2_democracy/tot/instiy.dta", keep(1 3) nogen keepus(z1dem1 z2dem2)
rename (z1dem1 z2dem2) (zdem1 zdem2)
forvalues s = 1/1 {
	forvalues x = 1/2 {
