...
do "${utils}/reghdfestd.ado"
do "${utils}/droptabular.ado"
do "${utils}/descstatistics.ado"

*Guard: autonomy_min/max must be set by 4_consolidate.do before 5_tables.do runs
cap assert !missing(${autonomy_min})
if _rc {
	noi di as error "FATAL: autonomy_min/max globals not defined. Run 4_consolidate.do first (or master.do)."
	exit 198
}
...
forv x=1/2{
	foreach y in $a {
		...
		noi di as error "DIAGNOSTIC Table1 IY y=`y' x=`x': N=`e(N)', singletons=`e(N_singletons)'"
...
forv x=1/2{
	foreach y in $a {
		...
		noi di as error "DIAGNOSTIC Table1 LT y=`y' x=`x': N=`e(N)', singletons=`e(N_singletons)'"
...
u "${temp}/ivs",replace
mer m:1 country year yearb using "${temp}/2_democracy/tot/demAge",keepus(dem1* dem2*) keep(1 3)
qui count if _merge==1
noi di as error "DIAGNOSTIC demAge (Table 2): `r(N)' ivs obs unmatched in demAge (no age-window exposure)"
drop _merge
forv s=1/1{
	forv c=1/1{ 
		forv x=2/2{
			...
			eststo: reghdfestd `y' demiy`x' dem`x'g4	[aw=weight] if age>32  , a(${fe`s'}) cl(${cl`c'}) version(5)  // age>32: ensures g4 (26-33) is always a complete 8-year window, symmetric with the IY window duration
			eststo: reghdfestd `y' demiy`x' dem`x'g5	[aw=weight] if age>40  , a(${fe`s'}) cl(${cl`c'}) version(5)  // age>40: ensures g5 (34-41) is always a complete 8-year window
...
u "${temp}/ivs",replace
keep if age>24
gen age_demeaned = age-25
egen mean_age_dm = mean(age_demeaned)
replace age_demeaned = age_demeaned-mean_age_dm
forv s=1/1{
	forv c=1/1{
		forv x=1/2{
			*Interaction uses demiy`x' so binary (x=1) and continuous (x=2) specs are self-consistent
			gen ageint = age_demeaned * demiy`x'
...
forv suc=1/4{
	u "${temp}/2_democracy/sucM/dem/suc`suc'iy",clear
	mer 1:m country yearb year using "${temp}/ivs",keep(3) nogen  // keep(3) only: restricts to country-years with suc coverage; unmatched ivs obs have missing sucdem and would be silently dropped from regression with keep(2 3)
...
gl saveout "${savee}/sucM/olsMain"
forv suc=1/4{
	u "${temp}/2_democracy/sucM/dem/suc`suc'lt",clear
	mer 1:m country yearb year using "${temp}/ivs",keep(3) nogen  // keep(3) only: restricts to country-years with suc coverage; unmatched ivs obs have missing sucdem and would be silently dropped from regression with keep(2 3)
	la var sucdem1 "Exposure to Successful Democracy"
	la var unsdem1 "Exposure to Unsuccessful Democracy"
	la var unsaut1 "Exposure to Unsuccessful Autocracy"
...
*=============================================================================*/
*A. Exposure.
u "${temp}/2_democracy/sucM/dem/suc1lt",clear 
keep country yearb year sucdem1 unsdem1 suc
foreach x in sucdem1 unsdem1 suc{
ren `x' `x'_growthlt
}
mer 1:1 country yearb year using "${temp}/2_democracy/sucM/dem/suc2lt",keep(2 3) nogen keepus(sucdem1 unsdem1 suc)
foreach x in sucdem1 unsdem1 suc{
ren `x' `x'_translt
}
mer 1:1 country yearb year using "${temp}/2_democracy/sucM/dem/suc3lt",keep(2 3) nogen keepus(sucdem1 unsdem1 suc)
foreach x in sucdem1 unsdem1 suc{
ren `x' `x'_caplt
}
mer 1:1 country yearb year using "${temp}/2_democracy/sucM/dem/suc4lt",keep(2 3) nogen keepus(sucdem1 unsdem1 suc)
foreach x in sucdem1 unsdem1 suc{
ren `x' `x'_redislt
}
mer 1:1 country yearb year using "${temp}/2_democracy/sucM/dem/suc1iy",keep(2 3) nogen keepus(sucdem1 unsdem1 suc)
foreach x in sucdem1 unsdem1 suc{
ren `x' `x'_growthiy
}
mer 1:1 country yearb year using "${temp}/2_democracy/sucM/dem/suc2iy",keep(2 3) nogen keepus(sucdem1 unsdem1 suc)
foreach x in sucdem1 unsdem1 suc{
ren `x' `x'_transiy
}
mer 1:1 country yearb year using "${temp}/2_democracy/sucM/dem/suc3iy",keep(2 3) nogen keepus(sucdem1 unsdem1 suc)
foreach x in sucdem1 unsdem1 suc{
ren `x' `x'_capiy
}
mer 1:1 country yearb year using "${temp}/2_democracy/sucM/dem/suc4iy",keep(2 3) nogen keepus(sucdem1 unsdem1 suc)
foreach x in sucdem1 unsdem1 suc{
ren `x' `x'_redisiy
}
...
*C. Pre-Birth.
u "${temp}/2_democracy/sucM/bef/suc1",clear
keep country yearb sucdem1 unsdem1 suc
foreach x in sucdem1 unsdem1 suc{
ren `x' `x'_growthlt
}
mer 1:1 country yearb using "${temp}/2_democracy/sucM/bef/suc2",keep(2 3) nogen keepus(sucdem1 unsdem1 suc)
foreach x in sucdem1 unsdem1 suc{
ren `x' `x'_translt
}
mer 1:1 country yearb using "${temp}/2_democracy/sucM/bef/suc3",keep(2 3) nogen keepus(sucdem1 unsdem1 suc)
foreach x in sucdem1 unsdem1 suc{
ren `x' `x'_caplt
}
mer 1:1 country yearb using "${temp}/2_democracy/sucM/bef/suc4",keep(2 3) nogen keepus(sucdem1 unsdem1 suc)
foreach x in sucdem1 unsdem1 suc{
ren `x' `x'_redislt
}
mer 1:m country yearb using "${temp}/2_democracy/tot/bef",keep(2 3) nogen
mer 1:m country yearb year using "${temp}/ivs",keep(2 3) nogen
la var bef1 "Pre-Birth Exposure to Democracy (Dichotomous)"
la var bef2 "Pre-Birth Exposure to Democracy (Continous)"
la var sucdem1_growthlt "Pre-Birth Exposure to Successful Democracy - Growth"
la var sucdem1_translt "Pre-Birth Exposure to Successful Democracy - Transparency"
la var sucdem1_caplt "Pre-Birth Exposure to Successful Democracy - Capacity"
la var sucdem1_redislt "Pre-Birth Exposure to Successful Democracy - Redistribution"
gl vardesc bef1 bef2 sucdem1_growthlt sucdem1_translt sucdem1_caplt sucdem1_redislt
descstatistics ${vardesc}, dec(2) saving("${savet}/statsprebirth.tex")

*D. Placebo.
u "${temp}/ivs",replace
mer 1:1 svyid using "${temp}/1_survey/placebo",nogen keep(1 3)
...
drop sortlen
expand 2 if _n == _N
replace variable = "__SENTINEL__" if _n == _N  // sentinel row controls two-pass token replacement
replace label = "__SENTINEL__" if _n == _N
...
do "${utils}/droptabular.ado"
u "${temp}/ivs",replace
mer m:1 country yearb year using "${temp}/2_democracy/tot/instiy.dta",keep(1 3) nogen keepus(z1dem1 z2dem2)
...
do "${utils}/droptabular.ado"
u "${temp}/ivs", replace
mer m:1 country yearb year using "${temp}/2_democracy/tot/instlt.dta", keep(1 3) nogen keepus(z1dem1 z2dem2)
ren (z1dem1 z2dem2) (zdem1 zdem2)
forv x=1/2{
    la var zdem`x' "Exposure to Democracy Wave t-1"
...
*=============================================================================*/
use "${temp}/ivs", clear
merge m:1 country year yearb using "${temp}/2_democracy/event/cohortsEvent", keep(1 3) nogen
...
use "${temp}/ivs", clear
merge m:1 country year yearb using "${temp}/2_democracy/event/cohortsEventValue5", keep(1 3) nogen
drop totdemevent
...
*PANEL A
u "${temp}/ivs",clear
mer m:1 country year yearb using "${temp}/2_democracy/event/cohortsEvent",keep(1 3) nogen //1 are cohorts for which demlt1 is not defined
...
*PANEL B
use "${temp}/2_democracy/event/cohortsEventValue5",clear
sa temp,replace
...
*=============================================================================*/
*WOOLRIDGE NUMBERS
use "${temp}/2_democracy/event/eventsDem1", clear
count
use "${temp}/2_democracy/event/cohortsEventValue5", clear
ds demevent*
