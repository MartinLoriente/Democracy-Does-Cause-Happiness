...
forv d=1/2{
	preserve
	u "${temp}/2_democracy/panelDem`d'",clear
	mer m:1 country using "${temp}/2_democracy/censusCountry",keepus(region regionanrr) nogen keep(1 3)
	...
	*RECOVER DROPPED COUNTRIES
	...
	ren demreg1* demreg*
	ren demregl* z*dem`d'
	drop dem regionanrr initialregime
...
sa "${temp}/2_democracy/panelInst",replace
...
* Growth
use country year gdppc2gr using "${temp}/3_addVars/gdp/gdp.dta", clear
...
use "${temp}/3_addVars/corruption", clear
gen t0 = 1 - corruption
...
* Capacity
use "${temp}/3_addVars/statecap", clear
...
*Redistribution
u "${temp}/3_addVars/redistribution",clear
...
so country year
compress
sa "${temp}/3_addVars/heM/he",replace
...
bys c0:g c1=_n
sa "${temp}/3_addVars/processors",replace

foreach w in $he_elements{
	u "${temp}/3_addVars/he`w'/he",replace
	...
	bys c0:g c1=_n
	compress
	sa "${temp}/3_addVars/he`w'/processors",replace
}
...
forv d=1/2{
	mer m:1 country year using "${temp}/2_democracy/panelDem`d'",keep(1 3) keepus(dem)
	qui count if _merge==1
	noi di as error "DIAGNOSTIC panelDem`d' (B1): `r(N)' census rows unmatched (no dem data for these country-years)"
	drop _merge
	ren dem dem`d'
}
...
compress
sa "${temp}/2_democracy/tot/dem",replace
...
compress
sa "${temp}/2_democracy/tot/bef",replace
...
expand yearsvy-yearb+1
bys id:g year=yearb-1+_n
mer m:1 country year using "${temp}/2_democracy/panelInst",keep(1 3) nogen
...
compress
sa "${temp}/2_democracy/tot/instiy",replace
...
la da "Instruments, exposure to democracy of neighbors"
compress
sa "${temp}/2_democracy/tot/instlt",replace
...
expand yearmax-yearmin+1
bys id:g year=yearmin-1+_n
mer m:1 country year using "${temp}/2_democracy/panelInst",keep(1 3) nogen keepus(z1dem1 z1dem2)
...
compress
sa "${temp}/2_democracy/tot/instBef",replace
...
args word
qui forv suc=1/`=_N'{
	u if c0==${pll_instance}&c1==`suc' using "${temp}/3_addVars/he`word'/processors",clear
	assert _N==1
	loc suci=suc in 1
	...
	compress
	sa "${temp}/2_democracy/suc`word'/dem/suc`suci'iy",replace
}
end
...
qui foreach w in $he_elements {
	...
	mer m:1 country year using "${temp}/3_addVars/he`w'/he",nogen keep(1 3)
	sort id year
	ds suc*
	...
	forv i=1/`nvars' {
		...
		*LOCF: forward-fill only actual missing gaps; preserves year-by-year suc variation
		by id: replace suc`i' = suc`i'[_n-1] if suc`i'==.
		la var suc`i' "`labelsuc`i''"
	}
	keep if year-yearb>=18&year-yearb<26 // IY
	*Backward-fill: carry earliest known suc value into pre-coverage years (e.g. pre-1980 for redistribution)
	gsort id -year
	forv i=1/`nvars' {
		by id: replace suc`i' = suc`i'[_n-1] if suc`i'==.
	}
	...
	u "${temp}/3_addVars/he`w'/processors",replace
	qui su c0
	...
	if `r(pll_errs)' > 0 { noi di as error "Parallel worker failed in C1. Aborting."; exit 1 }
}
...
qui forv suc=1/`=_N'{
	...
	compress
	sa "${temp}/2_democracy/suc`word'/dem/suc`suci'lt",replace // lo guardamos como LT
}
end
...
foreach w in $he_elements{
	...
	*Backward-fill: carry earliest known suc value into pre-coverage years (e.g. pre-1980 for redistribution)
	...
	noi parallel, prog(tempprocess): tempprocess `w'
	noi di as input "child processes which stopped with an error: " `r(pll_errs)'
	if `r(pll_errs)' > 0 { noi di as error "Parallel worker failed in C2. Aborting."; exit 1 }
}
...
*=============================================================================*/
/*NOTE ON VARIABLE NAMES IN sucM/bef/ FILES:
  dem1/dem2 here represent PRE-BIRTH democracy exposure (not lifetime or IY).
  aut1/aut2 represent pre-birth autocracy exposure (= 1 - dem1/2).
  The variable names are reused from the panel for consistency with tempprocess.
  In 5_tables.do, regressions on these files use dem1/dem2 as the regressor,
  which corresponds to bef1/bef2 in the conceptual framework of the paper.
*/
/*==============================================================================
...
qui forv suc=1/4{	
	...
	la var uns "Exposure to Unsuccessful Performance"
	compress
	sa "${temp}/2_democracy/sucM/bef/suc`suc'",replace
}
...
qui forv suc=1/`=_N'{
	...
	la var uns "Exposure to Unsuccessful Performance"
	compress
	sa "${temp}/2_democracy/suc`word'/inst/suc`suci'",replace
}
end
...
qui foreach w in $he_elements{
	...
	noi parallel, prog(tempprocess): tempprocess `w'
	noi di as input "child processes which stopped with an error: " `r(pll_errs)'
	if `r(pll_errs)' > 0 { noi di as error "Parallel worker failed in C6. Aborting."; exit 1 }
}
...
qui forv suc=1/`=_N'{
	...
	la var uns "Exposure to Unsuccessful Performance"
	compress
	sa "${temp}/2_democracy/suc`word'/inst/suc`suci'iy",replace
}
end
...
qui foreach w in $he_elements{
	...
	noi parallel, prog(tempprocess): tempprocess `w'
	noi di as input "child processes which stopped with an error: " `r(pll_errs)'
	if `r(pll_errs)' > 0 { noi di as error "Parallel worker failed in C7. Aborting."; exit 1 }
}
...
qui forv suc=1/`=_N'{
	...
	la var uns "Exposure to Unsuccessful Performance"
	compress
	sa "${temp}/2_democracy/suc`word'/instBef/suc`suci'",replace
}
end
...
qui foreach w in $he_elements{
	...
	noi parallel, prog(tempprocess): tempprocess `w'
	noi di as input "child processes which stopped with an error: " `r(pll_errs)'
	if `r(pll_errs)' > 0 { noi di as error "Parallel worker failed in C8. Aborting."; exit 1 }
}
...
compress
sa "${temp}/2_democracy/tot/demAge",replace
