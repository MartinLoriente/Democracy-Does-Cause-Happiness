...
*=============================================================================*/
usezipped using "raw/2democracy/AcemogluNaiduRestrepoRobinson2019/DDCGdata_final.zip",clear
...
u "raw/3addVars/population",replace
keep if year==1995
mer 1:m country using "temp",keep(2 3) nogen keepus(country region)
keep country region pop
duplicates drop
drop if pop<1000000
mer 1:1 country using "temp/1survey/censusCountry",keepus(country) nogen
keep country region
...
compress
sa "temp/2democracy/censusCountry",replace
...
use "raw/2democracy/FOfWv2.dta",clear
...
u "raw/2democracy/Polity2018.dta",clear
...
*=============================================================================*/
u "raw/2democracy/ddrevisited_data_v1",clear
...
*=============================================================================*/
u "raw/2democracy/bmr.dta",clear
ren democracy demBMR
keep country year demBMR
mer 1:1 country year using "temp",nogen
compress
sa "temp/2democracy/allSources",replace
...
u "temp",replace
mer 1:1 country year using "temp/2democracy/allSources",keep(1 3) keepus(status polity2 cl pr) nogen
...
compress
sa "temp/2democracy/panelDem1",replace
...
usezipped using "raw/2democracy/V-Dem-CY-Full+Others-v14.zip",clear
...
compress
sa "temp/2democracy/panelDem2",replace
...
*B1.1 WORLD BANK
*=============================================================================*/
u "raw/3addVars/1gdp/WorldBank.dta",replace
...
compress
sa "temp/3addVars/gdp/worldBank",replace

/*==============================================================================
*B1.2 UNSTATS
*=============================================================================*/
import excel using "raw/3addVars/1gdp/UNStats.xlsx",clear first case(l)
...
replace country="CHINA" if country=="CHINA, HONG KONG SAR"
replace country="SYRIA" if country=="SYRIAN ARAB REPUBLIC"
replace country="SAMOA (WESTERN SAMOA)" if country=="SAMOA"
...
compress
sa "temp/3addVars/gdp/unStats",replace

/*==============================================================================
*B1.3 MADISON
*=============================================================================*/
u "raw/3addVars/1gdp/GdpMaddison",clear
keep if year>=1840
...
replace country="COTE DIVOIRE" if country=="COTE D'IVOIRE"
compress
sa "temp/3addVars/gdp/madison",replace

/*==============================================================================
*B1.4 MERGE
*=============================================================================*/
u "temp/3addVars/gdp/unStats",replace
...
*WORLD BANK
u "temp/3addVars/gdp/worldBank",replace
...
u "temp/3addVars/gdp/madison",replace
forv g=1/7{
	preserve
	di `"keep if country==`"`t2'"'"'
...
drop t0 gdppc2010
mer m:1 country using "temp/2democracy/censusCountry",nogen keep(3) keepus(country)
so country year
drop if gdppc2011==.
egen t0=group(country)
xtset t0 year
...
u "temp/3addVars/gdp/unStats",replace
mer 1:1 country year using "temp/3addVars/gdp/worldBank",keepus(gdppc_2010 gdppc_gr gdppc_lcu) nogen
mer m:1 country using "temp/2democracy/censusCountry",nogen keep(3) keepus(country)
egen t0=group(country)
xtset t0 year
forv i=1/30{
...
compress
sa "temp/3addVars/gdp/gdp",replace
...
compress
sa "temp/3addVars/corruption",replace
...
use "raw/2democracy/V-Dem-CY-Full+Others-v14.dta",clear
mer 1:m country_name year using "raw/2democracy/statecap_finalv14",keep(2 3) nogen 
...
compress
sa "temp/3addVars/statecap",replace


/*==============================================================================
*B4. REDISTRIBUTION
*=============================================================================*/
usezipped using "raw/3addVars/5redistribution/wid/Data",clear
...
mer m:1 alpha2 using "raw/3addVars/5redistribution/wid/CountryCodes",keepus(country region) nogen keep(1 3) assert(2 3)
drop if region==""
drop alpha2 region
g sharetop=.
...
compress
sa "temp/3addVars/redistribution",replace
...
*B5. ESS
*=============================================================================*/
u "raw/1survey/6ess/ESS", replace
...
la data "ESS"
compress
sa "temp/3addVars/ess",replace

/*==============================================================================
*B5.2. COUNTRY-YEARB-YEAR ESS
*=============================================================================*/
*Country-Year born-Year
u "temp/3addVars/ess",clear
...
compress
sa "temp/1survey/censusCountryYearbYearess",replace